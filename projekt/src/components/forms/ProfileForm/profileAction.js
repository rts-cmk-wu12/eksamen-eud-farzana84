...
export default async function profileAction(prevState, formData) {
  ...
  if (!token || !userId) return { success: false, errors: ["Please sign in to continue."] };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { success: false, errors: [text || "Update failed, please try again"] };
    }
  } catch (err) {
    console.error("Network error calling:", url, err);
    return { success: false, errors: ["Network error. Check API_BASE_URL and server."] };
  }
